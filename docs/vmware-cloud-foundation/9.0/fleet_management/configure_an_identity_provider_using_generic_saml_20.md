---
source_url: https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/fleet-management/what-is/setting-up-sso/cofigure-vmware-cloud-foundation-identity-provider/configure-a-vcf-identity-provider-using-saml.html
product: vmware-cloud-foundation
version: 9.0
section: Fleet Management
breadcrumb: Fleet Management > Configure an Identity Provider using Generic SAML 2.0
---

# Configure an Identity Provider using Generic SAML 2.0

After installing or upgrading to VMware Cloud Foundation 9.0, to configure VCF Single Sign-On, you can configure an identity provider using Generic SAML 2.0 as the authentication protocol.
...
3. From the Choose Identity Provider section, select Generic SAML 2.0 from the list and click Next.
4. From the Configure the Identity Provider section, click Configure to configure the VCF Identity Broker to integrate with the selected identity provider for user authentication.
5. In the Authentication Method screen, select SAML (Security Assertion Markup Language).
6. In the Service Provider Registration screen, log in to your Identity Provider admin console and create a SAML 2.0 application.

   While configuring the application, use the metadata provided in the Service Provider Registration screen for the Entity ID/Metadata URL and Assertion Consumer Service URL options. If the identity provider requires the metadata to be copy-pasted, it is recommended to download this file to avoid formatting issues.
7. Click Next.
8. In the Identity Provider Configuration screen, enter the relevant details or select the required options and click Next.
   ...
   | Metadata URL | Copy the Metadata URL of your identity provider and paste it in this field.  The Metadata URL is used to fetch identity provider metadata. This metadata includes important information required for establishing trust and facilitating secure communication between the service provider and the identity provider.  If your identity provider is not publicly accessible or uses a self-signed certificate, VCF Operations cannot validate the metadata URL. Provide the metadata in XML format instead. |
...
9. In the User/Group Provisioning Method screen, select your preferred mode of provisioning the users and groups. You can select either:
   ...
   2. If you select JIT and click Next, complete the configuration as follows:
      ...
      | Just-In-Time Group Provisioning | This option creates groups dynamically based on the SAML assertion when the user logs in.  Alternatively, you can use group pre-provisioning if you know the group names from Active Directory that requires entitlements. |
   ...
   3. If you select AD/LDAP and click Next, complete the configuration as follows. To learn more about the benefits of using AD/LDAP as a provisioning method, see [KB 386870](https://knowledge.broadcom.com/external/article?articleId=386870).

      - In the Directory Details screen, enter the following details or select the required options.
        ...
        | Encrypted Connection  (Displayed when you select the DNS server location option) | This option activates LDAPs for all connections to this directory. |
...
10. In the Review screen, review the details you have added for the configuration and click Finish.
11. **Configure user and group provisioning**
12. In the Configure User and Group Provisioning screen, depending on the option you choose, complete the following steps and click Done.
    ...
    3. If you have chosen AD/LDAP, review the directory information such as the domain controller and bind credentials that you had configured. Complete the following steps:
       ...
       2. In the Group Provisioning screen, enter the relevant details or select the required options and click Next.
          ...
          | Sync Nested Groups | Select Sync Nested Group to sync all the users from the selected group and its nested group without maintaining the group hierarchy.  For large AD groups, do not select this option. Instead, you must manually select the required groups. |
...
13. Click Finish Setup.
    ...
    **Important Notes on Role Assignment**
    ...
    Log in as a local admin to each component and assign service roles to provisioned users/groups to activate VCF Single Sign-On. Access the list of components under the 'Component Configuration' tab at the end of the setup.