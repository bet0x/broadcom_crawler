---
source_url: https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/fleet-management/what-is/setting-up-sso/cofigure-vmware-cloud-foundation-identity-provider/configure-identity-provider-for-okta/configure-a-identity-provider-with-okta-using-saml.html
product: vmware-cloud-foundation
version: 9.0
section: Fleet Management
breadcrumb: Fleet Management > Configure Okta as an Identity Provider using OIDC
---

# Configure Okta as an Identity Provider using OIDC

After installing or upgrading to VMware Cloud Foundation 9.0, to configure VCF Single Sign-On, you can configure Okta as an identity provider using OIDC as the authentication protocol.
...
- If you configure VCF Single Sign-On using VCF Identity Broker appliance, and there are network restrictions that do not allow outbound public internet access, you must set up HTTP proxy. For more information, [KB 393078](https://knowledge.broadcom.com/external/article?articleId=393078).
...
3. For a fresh identity provider configuration, from the Choose Identity Provider section, select Okta from the list and click Next.
...
7. In the Identity Provider Configuration screen, enter the relevant details or select the required options and click Next.
   ...
   | --- | --- |
   | **Option** | **Description** |
   | IDP Display Name | Enter a friendly name for your identity provider. Example, Okta. |
   ...
   | Redirect URI | The URL where the identity provider sends the authentication response after a user logs in. |
8. In the User/Group Provisioning Method screen, select your preferred mode of provisioning the users and groups. You can select either:
   ...
   1. If you select SCIM and click Next, complete the configuration as follows.
      ...
      - In the Domain(s) screen, enter the domains that users from your enterprise will log in with, and click Next.
   2. If you select JIT and click Next, complete the configuration as follows:
      ...
      | Just-In-Time Group Provisioning | This option creates groups dynamically based on the OIDC token when the user logs in.  Alternatively, you can use group pre-provisioning if you know the group names from Active Directory that requires entitlements. |
   ...
   3. If you select AD/LDAP and click Next, select Unique Identifier in OIDC Identity Provider and Unique Identifier in VCF Identity Broker and click Next. To learn more about the benefits of using AD/LDAP as a provisioning method, [KB 386870](https://knowledge.broadcom.com/external/article?articleId=386870).
      ...
      - In the Directory Details screen, enter the relevant details or select the required options.

        |  |  |
        | --- | --- |
        | **Option** | **Description** |
        | Directory name | Enter a name for the directory.  The directory name should contain only letters, digits, spaces, hyphens, and underscores. |
