---
source_url: https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/fleet-management/what-is/setting-up-sso/cofigure-vmware-cloud-foundation-identity-provider/configure-identity-provider-for-ping-federate/configure-identity-provider-for-ping-federate-using-oidc.html
product: vmware-cloud-foundation
version: 9.0
section: Fleet Management
breadcrumb: Fleet Management > Configure Ping Identity as an Identity Provider Using OIDC
---

# Configure Ping Identity as an Identity Provider Using OIDC

After installing or upgrading to VMware Cloud Foundation, to configure VCF Single Sign-On, you can configure Ping Identity as an identity provider using OIDC as the authentication protocol.
...
3. For a fresh identity provider configuration, from the Choose Identity Provider section, select Ping Identity from the list and click Next.
...
7. In the Identity Provider Configuration screen, enter the relevant details or select the required options and click Next.
   ...
   | --- | --- |
   | **Option** | **Description** |
   | IDP Display Name | Enter the OpenID Connect provider name. Example, Ping Identity. |
   ...
   | SSL Certificate | A public key certificate to verify the OIDC identity. |
8. In the User/Group Provisioning Method screen, select your preferred mode of provisioning the users and groups. You can select either:
   ...
   3. If you select AD/LDAP and click Next, select Unique Identifier in OIDC Identity Provider and Unique Identifier in VCF Identity Broker and click Next. To learn more about the benefits of using AD/LDAP as a provisioning method, see [KB 386870](https://knowledge.broadcom.com/external/article?articleId=386870).
...
11. In the Configure User and Group Provisioning screen, depending on the option you choose, complete the following steps and click Done.
    ...
    If you have chosen OIDC as the authentication protocol, and AD/LDAP as the user and group provisioning protocol, read [KB 3868](https://knowledge.broadcom.com/external/article/386870)70 to understand the finer details about configuring the setup.
