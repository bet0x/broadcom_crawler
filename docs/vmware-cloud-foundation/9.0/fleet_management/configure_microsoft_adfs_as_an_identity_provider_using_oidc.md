---
source_url: https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/fleet-management/what-is/setting-up-sso/cofigure-vmware-cloud-foundation-identity-provider/configure-identity-provider-for-microsoft-adfs-using-saml/configure-identity-provider-for-microsoft-adfs-using-oidc.html
product: vmware-cloud-foundation
version: 9.0
section: Fleet Management
breadcrumb: Fleet Management > Configure Microsoft ADFS as an Identity Provider Using OIDC
---

# Configure Microsoft ADFS as an Identity Provider Using OIDC

After installing or upgrading to VCF 9.0, to configure VCF Single Sign-On, you can configure Microsoft ADFS as an identity provider using OIDC as the authentication protocol.
...
3. For a fresh identity provider configuration, from the Choose Identity Provider section, select Microsoft Entra ADFS from the list and click Next.
...
7. In the Identity Provider Configuration screen, enter the relevant details or select the required options and click Next.
   ...
   | --- | --- |
   | **Option** | **Description** |
   | IDP Display Name | Enter the OpenID Connect provider name. Example, Microsoft Entra ADFS. |
   ...
   | OpenID Address | The URL of the identity provider’s OIDC configuration end point for initiating the authentication process. |
   | Redirect URI | The URL where the identity provider will send the authentication response after a user logs in. |
   | SSL Certificate | Public key certificate to verify OIDC identity.  This field is required only if a self-signed certificate is used to sign the identity provider. |
8. In the User/Group Provisioning Method screen, select your preferred mode of provisioning the users and groups. You can select either:
   ...
   2. If you select AD/LDAP, select the Unique Identifier in OIDC Identity Provider and Unique Identifier in VCF Identity Broker, and click Next. To learn more about the benefits of using AD/LDAP as a provisioning method, see [KB 386870](https://knowledge.broadcom.com/external/article?articleId=386870).
...
11. In the Configure User and Group Provisioning screen, click Done.
...
12. Click Finish Setup.
    ...
    **Important Notes on Backup**
    ...
    It is highly advisable to create a backup of both the management domain vCenter and the VCF Identity Broker appliance, based on the deployment option selected. Furthermore, it is advisable to perform a similar backup following any substantial modifications to the initial identity source configuration, such as when you 'Reset SSO'. This practice ensures that you can revert the VCF SSO configuration to a recognized and stable state if necessary.

    **Important Notes on Role Assignment**

    Log in as a local admin to each component and assign service roles to provisioned users/groups to activate VCF SSO. Access the list of components under the 'Component Configuration' tab at the end of the setup. For more information, see [Step 7. Assigning Roles and Permissions](/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/fleet-management/what-is/setting-up-sso/assigning-roles-and-permissions.html#GUID-40a7ccb8-1e83-4926-b7ef-2365a865f889-en_id-5e7caad9-e3b7-41d2-f85d-f7cf7a76a0d5).