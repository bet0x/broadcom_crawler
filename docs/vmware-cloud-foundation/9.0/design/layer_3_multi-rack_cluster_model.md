---
source_url: https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/design/design-library/cluster-models/multi-rack-cluster-detailed-design/layer-3-multi-rack-cluster.html
product: vmware-cloud-foundation
version: 9.0
section: Design
breadcrumb: Design > Layer 3 Multi-Rack Cluster Model
---

# Layer 3 Multi-Rack Cluster Model

The Layer 3 Multi-Rack Cluster Model allows you to protect workloads on vSphere clusters in your environment against a failure of a single fault domain by implementing your vSphere cluster across multiple fault domains within a single availability zone. The networks are isolated to a rack using a separate broadcast domain for each network per rack.

The Layer 3 Multi-Rack Cluster Model incorporates multiple fault domains in the design that can help reduce the blast radius of a failure which can increase application availability. In this design the VCF infrastructure networks are not stretched across racks, instead each rack has a separate VLAN and subnet. The network traffic is routed between the racks, this increases the number of VLANs and subnets needed for the layer 3 multi-rack deployment but also uses a single broadcast domain for each network thus reducing the impact of a layer 2 network issue across racks.

Layer 3 Multi-Rack Cluster

![](/content/broadcom/techdocs/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/_jcr_content/assetversioncopies/31921790-fd33-4272-a074-819c2823372c.original.svg)

## Layer 3 Multi-Rack Cluster Model Attributes

A Layer 3 Multi-Rack Cluster Model has the following attributes.

| Attribute | Detail |
| --- | --- |
| Datacenters | Single datacenter |
| Cluster Rack Mapping | Hosts are installed horizontally across different physical racks. |
| Management Domain Clusters | Not supported for primary cluster, additional clusters supported |
...
| Networking | - Layer 3 routed networks with isolated broadcast domains per rack |
| Storage | Storage capable of surviving the failure of a single fault domain at the storage level. |

## Layer 3 Multi-Rack Cluster Model VLANs and Subnets

| Function | Network Attributes |
| --- | --- |
| VM management | - VLAN local to the ToR switches per rack - Separate subnet per rack - Highly available gateway at the ToR switches - Routable network across racks - \*Depends on NFS storage fabric network design |
| ESX management |
| vSphere vMotion |
| vSAN |
| Host overlay |
| NFS\* |
| NSX Edge Uplink01 | - See NSX edge node design for details [Rack Fault Tolerant using a Layer 3 Multi-Rack Cluster NSX Edge Cluster Model](/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/design/design-library/design-library-nsx-edge-cluster/nsx-edge-cluster-models-for-single-availability-zone/rack-fault-tolerant-design-single-layer-3-vsphere-clusters-deployed-across-multiple-racks.html) |
| NSX Edge Uplink02 |
| NSX Edge overlay |

## Layer 3 Multi-Rack Cluster Model Sizing Considerations

You choose the number of ESX hosts per vSphere cluster based on storage type and workload domain considerations.

| Attribute | Storage Type | Management Domain (Additional Clusters)  Workload Domain (All Clusters) |
| --- | --- | --- |
| Minimum number of ESX hosts | vSAN | Three (3) |
| VMFS on FC\*  NFS\* | Two (2) |
...
## Layer 3 Multi-Rack Cluster Model Options

A Layer 3 Multi-Rack Cluster Model can leverage options from the design areas listed below, depending on objectives.
...
## Layer 3 Multi-Rack Cluster Virtual Networking Design Requirements

Layer 3 Multi-Rack Cluster Virtual Networking Infrastructure Design Requirements

Virtual networking infrastructure requirements for a layer 3 multi-rack cluster describe the design requirements for the configuration of NSX host transport nodes in the cluster to provide virtual networking capabilities including VPC networking and NSX segment networking.



| Design Requirement ID | Design Requirement | Justification | Implication |
| --- | --- | --- | --- |
| VCF-VNI-L3MR-REQD-CFG-001 | Create an uplink profile for each rack. | Enables a Layer 3 boundary at the top-of-rack switches for host TEP traffic. | Requires additional subnet per rack. |
| VCF-VNI-L3MR-REQD-CFG-002 | Create a host TEP IP pool for each rack with a subnet allocated per rack and a gateway for the subnet. | - Provides the host TEP IP addressing for each rack - Enables a Layer 3 boundary at the top-of-rack switches for host TEP traffic. - Provides routing capabilities for the host TEPs between racks. | Requires additional subnet per rack. |
| VCF-VNI-L3MR-REQD-CFG-003 | Create NSX host sub transport node profiles for additional racks. | Enables NSX host transport node configuration per rack. | None. |

## Layer 3 Multi-Rack Cluster Virtual Networking Design Recommendations

Layer 3 Multi-Rack Cluster Virtual Networking Infrastructure Design Recommendations

Virtual networking infrastructure recommendations for a layer 3 multi-rack cluster describe the design requirements for the configuration of NSX host transport nodes in the cluster to provide virtual networking capabilities including VPC networking and NSX segment networking.



| Design Recommendation ID | Design Recommendation | Justification | Implication |
| --- | --- | --- | --- |
| VCF-VNI-L3MR-REQD-CFG-001 | Use a separate host TEP transport VLAN ID per rack. | Enables a Layer 3 boundary at the top-of-rack switches for host TEP traffic. | Requires additional host TEP VLAN ID per rack. |