---
source_url: https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/design/design-library/design-library-nsx-edge-cluster/nsx-edge-cluster-models-for-single-availability-zone/host-fault-tolerant-nsx-edge-cluster-design.html
product: vmware-cloud-foundation
version: 9.0
section: Design
breadcrumb: Design > Host Fault Tolerant NSX Edge Cluster Model
---

# Host Fault Tolerant NSX Edge Cluster Model

A [Single-Rack Cluster Model](/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/design/design-library/cluster-models/single-instance-single-availability-zone.html) for hosting NSX Edge nodes offers a minimalist footprint while maintaining basic host-level availability.

**Host Fault Tolerant NSX Edge Cluster Model**

![](/content/broadcom/techdocs/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/_jcr_content/assetversioncopies/a0cc745e-2ba1-48ec-8acb-f51a10603072.original.png)

## Host Fault Tolerant NSX Edge Cluster Model Attributes

The Host Fault Tolerant NSX Edge Cluster Model has the following attributes.

| Attribute | Details |
| --- | --- |
| Availability model | - Host level redundancy only. - Single shared storage. - This design cannot ensure continuous service availability for north-south communication and centralized network services during a complete rack failure. |
| NSX Edge node mobility across racks | N/A |
...
| North-South bandwidth growth | - Add hosts to the single existing vSphere cluster or preferably transition to a rack fault tolerant design with two or more vertical Layer 2 vSphere cluster in separate rack. - When deploying more NSX Edge nodes on existing hosts, ensure each host has sufficient CPU, memory, and importantly, additional physical NICs (pNICs) to maintain maximum packet forwarding rates (PPS) for each NSX Edge node. |
