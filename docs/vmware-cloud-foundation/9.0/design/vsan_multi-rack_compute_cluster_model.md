---
source_url: https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/design/design-library/storage-models(1)/standard-vsan-storage-model/vsan-compute-cluster-storage-models/vsan-multirack-compute-cluster-model.html
product: vmware-cloud-foundation
version: 9.0
section: Design
breadcrumb: Design > vSAN Multi-Rack Compute Cluster Model
---

# vSAN Multi-Rack Compute Cluster Model

A vSAN multi-rack compute cluster consumes storage from a vSAN storage cluster deployed in the same workload domain. A vSAN compute cluster can be spread over multiple racks routed via layer 3 networking. Similar to the Layer 3 Multi-Rack Cluster Model this allows protection of workloads on vSphere clusters in your environment against a failure of a single fault domain by implementing a vSAN compute cluster across multiple fault domains within a single availability zone. The networks are isolated to a rack using a separate broadcast domain for each network per rack.

vSAN Multirack compute cluster.

![](/content/broadcom/techdocs/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/_jcr_content/assetversioncopies/aaec4cf0-4308-49d2-a54a-52dc5f618872.original.svg)

vSAN Multi-Rack Compute Cluster Attributes
...
| Cluster Rack Mapping | Hosts are deployed horizontally across different physical racks |
...
| vSAN compute cluster | - A vSAN Compute Cluster is a [vSphere Cluster Models](/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/design/vmware-cloud-foundation-concepts/vsphere-cluster-models.html) with a [vSAN Traffic Type](/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/design/design-library/storage-models(1)/standard-vsan-storage-model/vcf-network-design-of-vsan-storage.html#GUID-7f27d138-d6ab-493f-a316-cb57a01e2b08-en_id-6b5ac9d1-3397-4f8c-b56e-4453c86757bb) defined - The primary function of a vSAN compute cluster is to provide compute resources (CPU, memory) for running virtual machines, not storage. - A vSAN compute-only cluster can act as a client to a vSAN Storage cluster. - A vSAN compute cluster does not have to be a certified vSAN Ready node, thus does not require certified vSAN local storage devices for vSAN use. |
...
| vSAN remote datastore | vSAN compute cluster will mount a remote datastore from vSAN storage cluster. |

vSAN Multi-Rack Compute Cluster Model dependencies
...
vSAN Compute Multi-Rack Storage Cluster Design Recommendations



| Design Recommendation ID | Design Recommendation | Justification | Implication |
| --- | --- | --- | --- |
| VCF-VSAN-COMP-L3MR-RCMD-CFG-001 | Use Layer 3 networking between each fault domain (physical rack). | Provides fault isolation per fault domain or physical rack. | - Requires multiple IPv4 subnet per rack. - Requires gateway per IPv4 subnet. - Requires layer 3 routing between racks. |