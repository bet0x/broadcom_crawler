---
source_url: https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/design/design-library/design-library-nsx-edge-cluster/nsx-edge-cluster-models-for-single-availability-zone/rack-fault-tolerant-design-single-layer-3-vsphere-clusters-deployed-across-multiple-racks.html
product: vmware-cloud-foundation
version: 9.0
section: Design
breadcrumb: Design > Rack Fault Tolerant using a Layer 3 Multi-Rack Cluster NSX Edge Cluster Model
---

# Rack Fault Tolerant using a Layer 3 Multi-Rack Cluster NSX Edge Cluster Model

A single horizontally deployed vSphere cluster for hosting NSX Edge nodes provides host-level and rack-level availability but without providing independent storage for each rack. The vSphere cluster is deployed with one or more hosts per rack across two or more racks, unlike the other designs with separated vertical vSphere clusters where each vSphere cluster was contained within a single rack. This design distributes hosts from the same vSphere cluster across multiple racks, with Layer 3 network boundaries providing separation between each rack.

**Rack Fault Tolerant using a Layer 3 Multi-Rack Cluster NSX Edge Cluster Model**

![](/content/broadcom/techdocs/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/_jcr_content/assetversioncopies/2c4b2048-0e7b-409d-9ad0-46ffbcc2fc92.original.png)

## Rack Fault Tolerant using a Layer 3 Multi-Rack Cluster NSX Edge Cluster Model Attributes

The Rack Fault Tolerant using a Layer 3 Multi-Rack Cluster NSX Edge Cluster Model has the following attributes.
...
## Rack Fault Tolerant using a Layer 3 Multi-Rack Cluster NSX Edge Cluster Model Design Requirements
...
| Design Requirement ID | Design Requirement | Justification | Implication |
| --- | --- | --- | --- |
| VCF-NSX-EDGE-REQD-RFTL3-01 | Deploy two (2) or more NSX Edge nodes evenly across racks. | - NSX Edge nodes are required to support centralized network connectivity for the virtual network. - At least two (2) NSX Edge nodes are required to support NSX Edge high-availability. | - Cluster resources must be available to support the NSX Edge nodes. - Additional NSX Edge nodes may be required to satisfy increased bandwidth requirements. |

## Rack Fault Tolerant using a Layer 3 Multi-Rack Cluster NSX Edge Cluster Model Design Recommendations
