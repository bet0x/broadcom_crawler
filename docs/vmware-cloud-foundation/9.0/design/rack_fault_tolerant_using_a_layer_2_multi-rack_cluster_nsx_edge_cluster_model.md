---
source_url: https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/design/design-library/design-library-nsx-edge-cluster/nsx-edge-cluster-models-for-single-availability-zone/rack-fault-tolerant-design-single-layer-2-vsphere-clusters-deployed-across-multiple-racks.html
product: vmware-cloud-foundation
version: 9.0
section: Design
breadcrumb: Design > Rack Fault Tolerant using a Layer 2 Multi-Rack Cluster NSX Edge Cluster Model
---

# Rack Fault Tolerant using a Layer 2 Multi-Rack Cluster NSX Edge Cluster Model

A single horizontally deployed vSphere cluster for hosting NSX Edge nodes provides host-level and rack-level availability but without providing independent storage for each rack. The vSphere cluster is deployed with one or more hosts per rack across two or more racks, unlike the other designs with separated vertical vSphere clusters where each vSphere cluster was contained within a single rack. Unlike the previous horizontal design, this design distributes hosts from the same vSphere cluster within the same Layer 2 domain between racks rather than across Layer 3 boundaries. The physical network fabric must support Layer 2 adjacencies (e.g., through VXLAN encapsulation) to enable connectivity between hosts across different racks.

**Rack Fault Tolerant using a Layer 2 Multi-Rack Cluster NSX Edge Cluster Model**

![](/content/broadcom/techdocs/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/_jcr_content/assetversioncopies/1b9e51cf-5cdb-4c13-8a19-ffcb4b899599.original.png)

## Rack Fault Tolerant using a Layer 2 Multi-Rack Cluster NSX Edge Cluster Model Attributes

The Rack Fault Tolerant using a Layer 2 Multi-Rack Cluster NSX Edge Cluster Model has the following attributes.

| Attribute | Details |
| --- | --- |
| Availability model | - Host and rack-level redundancy. - Single shared storage. - This design ensures continuous service availability for north-south communication and centralized network services during a complete rack failure, similar to the multiple vertical vSphere cluster design. |
| NSX Edge node mobility across racks | Supported, though implementing this capability is not required nor recommended. |
| BGP peering | Because BGP peering over extended networks may be problematic in some physical network fabrics, it's recommended to establish BGP peering only with local ToR switches to eliminate this dependency, even when Layer 2 adjacency is available across racks. |
...
| North-South bandwidth growth | - Add hosts to existing racks or extend the vSphere cluster to span additional racks, with NSX Edge nodes distributed evenly across the racks. - When deploying more NSX Edge nodes on existing hosts, ensure each host has sufficient CPU, memory, and importantly, additional physical NICs (pNICs) to maintain maximum packet forwarding rates (PPS) for each NSX Edge node. |
...
## Rack Fault Tolerant using a Layer 2 Multi-Rack Cluster NSX Edge Cluster Model Design Requirements
...
| Design Recommendation ID | Design Recommendation | Justification | Implication |
| --- | --- | --- | --- |
| VCF-NSX-EDGE-RCMD-RFTL2-01 | Ensure that NSX Edge nodes belonging to the same NSX Edge cluster are deployed evenly across different racks. | Maintain High-Availability and prevent single points of failure. | Additional servers are required. |
| VCF-NSX-EDGE-RCMD-RFTL2-02 | Configure an NSX failure domain for each rack where the NSX Edge node can run - this is only valid when you have more than two NSX Edge nodes; map the NSX Edge nodes to the NSX failure domains appropriately. | - Failure domains guarantee service availability in case of a failure affecting multiple NSX Edge nodes. - Active and standby instance of a VPC or Tier-1 gateway always run in different failure domains. | - Additional manual configuration. - This applies only for designs incorporating VPC or Tier-1 gateway in A/S mode. |